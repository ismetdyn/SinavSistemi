...
namespace SinavSistemi.BusinessLayer
{
    public class KullaniciVVL : AbstractValidator<Kullanici>
    {
        public KullaniciVVL()
        {
            ...
            RuleFor(x => x.KullaniciTipID).NotNull().WithMessage("Kullanıcı tipi null ifade olamaz");
            RuleFor(x => x.KullaniciTipID).NotEmpty().WithMessage("Kullanıcı tipi boş olamaz");
            RuleFor(x => x.KullaniciTipID).GreaterThan(0).WithMessage("Kullanıcı tipi 0' dan büyük olmalıdır.");
            RuleFor(x => x.KullaniciTipID).LessThan(4).WithMessage("Kullanıcı tipi 4'ten büyük olamaz");
